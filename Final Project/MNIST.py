...
from numpy import append, array, int8, uint8, zeros, set_printoptions
import numpy as np
...
if __name__ == '__main__':
    images, labels = load_mnist('training', path='/Users/Michael/Documents/UNCW/Jahr 4/Herbst 2016/CSC 415/Final Project/')
    # print("Images: {}\n".format(images))
    # print("Labels: {}\n".format(labels))

    n = 100
    mnist_array = np.zeros((n, images[0].size))
    for i in range(n):
        mnist_array[i] = np.ravel(images[i]) * 100

    np.savetxt('mnist.inp', mnist_array, '%f')

    # for r in range(0, 1):
    #     np.savetxt(''.join(('mnist', str(r + 1), '.inp')), np.ravel(images[r]), "%f")
